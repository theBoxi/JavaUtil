package ch.boxi.javaUtil.id.decorator.format;

import java.util.Map;
import java.util.Map.Entry;
import ch.boxi.javaUtil.id.ID;
import ch.boxi.javaUtil.id.decorator.format.IDFormat;
import ch.boxi.javaUtil.id.decorator.prefix.PrefixDecorator;

public class SimpleFormatRunner implements Runnable{
	
	private IDFormat format;
	private Map<ID, String> idToExpected;
	private SimpleFormatMultiThreadTest test;
	
	public SimpleFormatRunner(IDFormat format, Map<ID, String> idToExpected, SimpleFormatMultiThreadTest test){
		this.format = format;
		this.idToExpected = idToExpected;
		this.test = test;
	}
	
	@Override
	public void run() {
		for(Entry<ID, String> entry: idToExpected.entrySet()){
			String value = format.formatID(new PrefixDecorator(entry.getKey(), ""));
			if(!entry.getValue().equals(value)){
				test.errorFound();
			}
		}
	}
}
